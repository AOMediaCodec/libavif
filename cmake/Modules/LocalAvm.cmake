...
set(AVM_EXT_SOURCE_DIR "${AVIF_SOURCE_DIR}/ext/avm")
set(LIB_FILENAME "${AVM_EXT_SOURCE_DIR}/build.libavif/${CMAKE_STATIC_LIBRARY_PREFIX}avm${CMAKE_STATIC_LIBRARY_SUFFIX}")

if(EXISTS "${LIB_FILENAME}")
    message(STATUS "libavif(AVIF_CODEC_AVM=LOCAL): compiled library found at ${LIB_FILENAME}")
    add_library(avm STATIC IMPORTED GLOBAL)
    set_target_properties(avm PROPERTIES IMPORTED_LOCATION "${LIB_FILENAME}" AVIF_LOCAL ON)
    target_include_directories(avm INTERFACE $<BUILD_INTERFACE:${AVM_EXT_SOURCE_DIR}>)
    ...
    pkg_check_modules(_AVM QUIET avm)

    set(_AVM_PC_LIBRARIES "${_AVM_STATIC_LIBRARIES}")
    # remove "avm" so we only have library dependencies
    list(REMOVE_ITEM _AVM_PC_LIBRARIES "avm")
    # Work around crbug.com/aomedia/356153293: avm.pc has -lm in the
    # Libs.private field on all platforms, whether libm or m.lib exists or not.
    if(WIN32 OR APPLE)
        list(REMOVE_ITEM _AVM_PC_LIBRARIES "m")
    endif()

    # Add absolute paths to libraries
    foreach(_lib ${_AVM_PC_LIBRARIES})
        find_library(_avm_dep_lib_${_lib} ${_lib} HINTS ${_AVM_STATIC_LIBRARY_DIRS})
        target_link_libraries(avm INTERFACE ${_avm_dep_lib_${_lib}})
    endforeach()
else()
    message(STATUS "libavif(AVIF_CODEC_AVM=LOCAL): compiled library not found at ${LIB_FILENAME}, using FetchContent")
    if(EXISTS "${AVM_EXT_SOURCE_DIR}")
        message(STATUS "libavif(AVIF_CODEC_AVM=LOCAL): ext/avm found; using as FetchContent SOURCE_DIR")
        set(FETCHCONTENT_SOURCE_DIR_AVM "${AVM_EXT_SOURCE_DIR}")
        message(CHECK_START "libavif(AVIF_CODEC_AVM=LOCAL): configuring avm")
    else()
