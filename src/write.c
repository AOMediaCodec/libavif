...
{
    uint16_t id;
    uint8_t type[4];                      // 4-character ISOBMFF box type
    ...
    avifItemType itemType;                // Type of item being encoded
...
{
    return (itemType == AVIF_ITEM_ALPHA) ? AVIF_RESULT_ENCODE_ALPHA_FAILED : AVIF_RESULT_ENCODE_COLOR_FAILED;
}
                                              ...
                                              avifAddImageFlags addImageFlags)
{
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
        ...
        if (item->codec) {
            ...
            const int quantizer = (item->itemType == AVIF_ITEM_ALPHA) ? encoder->data->quantizerAlpha : encoder->data->quantizer;
            ...
            if (encodeResult == AVIF_RESULT_UNKNOWN_ERROR) {
                encodeResult = avifGetErrorForItemType(item->itemType);
            }
...
avifResult avifEncoderFinish(avifEncoder * encoder, avifRWData * output)
{
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
        ...
        if (item->codec) {
            if (!item->codec->encodeFinish(item->codec, item->encodeOutput)) {
                return avifGetErrorForItemType(item->itemType);
            }
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
        ...
        if (item->encodeOutput->samples.count > 0) {
            ...
            AVIF_CHECKERR(avifSequenceHeaderParse(&sequenceHeader, (const avifROData *)&firstSample->data, codecType),
                          avifGetErrorForItemType(item->itemType));
    ...
    for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
        ...
        avifItemPropertyDedupStart(dedup);
        uint8_t channelCount = ((item->itemType == AVIF_ITEM_ALPHA) || (imageMetadata->yuvFormat == AVIF_PIXEL_FORMAT_YUV400)) ? 1 : 3;
        ...
        if (item->itemType == AVIF_ITEM_ALPHA) {
    ...
    if (isSequence) {
        ...
        for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
            ...
            avifRWStreamWriteU32(&s, 0); // unsigned int(32) pre_defined = 0;
            avifRWStreamWriteChars(&s, (item->itemType == AVIF_ITEM_ALPHA) ? "auxv" : "pict", 4); // unsigned int(32) handler_type;
            avifRWStreamWriteZeros(&s, 12);           // const unsigned int(32)[3] reserved = 0;
            avifRWStreamWriteChars(&s, "libavif", 8); // string name; (writing null terminator)
            ...
            if (item->itemType == AVIF_ITEM_COLOR) {
    ...
    for (uint32_t itemPasses = 0; itemPasses < 3; ++itemPasses) {
        ...
        for (uint32_t itemIndex = 0; itemIndex < encoder->data->items.count; ++itemIndex) {
            ...
            if (alphaPass != (item->itemType == AVIF_ITEM_ALPHA)) {
            ...
            if ((encoder->extraLayerCount > 0) && (item->encodeOutput->samples.count > 0)) {
                ...
                avifEncoderItemReference * ref = (item->itemType == AVIF_ITEM_ALPHA) ? avifArrayPushPtr(&layeredAlphaItems)
