...
TEST_P(SampleTransformTest, Avif16bit) {
  ...
  // image_8msb = image >> shift
  ASSERT_EQ(
      avifImageTransformConstantAndImageSamples(
          image_no_sato.get(), AVIF_SAMPLE_TRANSFORM_INTERMEDIATE_BIT_DEPTH_32,
          /*leftOperand=*/1 << shift, AVIF_SAMPLE_TRANSFORM_DIVIDE_REVERSED,
          /*rightOperand=*/image.get(), AVIF_PLANES_ALL),
      AVIF_RESULT_OK);
