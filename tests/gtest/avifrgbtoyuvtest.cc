...
TEST_P(SetGetRGBATest, SetGetTest) {
  ...
  testutil::AvifImagePtr yuv(avifImageCreate(4, 2, 8, AVIF_PIXEL_FORMAT_YUV444),
                             avifImageDestroy);
  ...
  if (!avifGetRGBColorSpaceInfo(&rgb, &color_space)) {
    return;  // Invalid combination e.g. RGB565 with depth > 8.
  }

  float kEpsilon = 1.0f / color_space.maxChannelF;
  if (rgb_format == AVIF_RGB_FORMAT_RGB_565) {
    // Only 5 bits of information per channel except G which has 6).
    kEpsilon = 1.0f / (1 << 5);
  }

  const float pixel_to_write[4] = {0.0f, 0.5f, 1.0f, 0.75f};
  float pixel_read[4];
  for (uint32_t j = 0; j < rgb.height; ++j) {
    for (uint32_t i = 0; i < rgb.width; ++i) {
      avifSetRGBAPixel(&rgb, i, j, pixel_to_write, &color_space);
      avifGetRGBAPixel(&rgb, i, j, pixel_read, &color_space);
      EXPECT_NEAR(pixel_read[0], pixel_to_write[0], kEpsilon);
      EXPECT_NEAR(pixel_read[1], pixel_to_write[1], kEpsilon);
      EXPECT_NEAR(pixel_read[2], pixel_to_write[2], kEpsilon);
      if (avifRGBFormatHasAlpha(rgb_format)) {
        EXPECT_NEAR(pixel_read[3], pixel_to_write[3], kEpsilon);
      } else {
...
INSTANTIATE_TEST_SUITE_P(NonFloat, SetGetRGBATest,
                         Combine(/*rgb_depth=*/Values(8, 10, 12, 16),
                                 Range(AVIF_RGB_FORMAT_RGB,
                                       AVIF_RGB_FORMAT_COUNT),
                                 /*is_float=*/Values(false)));

INSTANTIATE_TEST_SUITE_P(Float, SetGetRGBATest,
                         Combine(/*rgb_depth=*/Values(8),
                                 Range(AVIF_RGB_FORMAT_RGB,
                                       AVIF_RGB_FORMAT_COUNT),
