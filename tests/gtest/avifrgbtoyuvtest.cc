...
void modifyImageChannel(avifRGBImage* image, avifChannel channel,
...
void setImageChannel(avifRGBImage* image, avifChannel channel, uint32_t value,
                     const int32_t modifier[]) {
...
void getDiffSumAndSqDiffSum(const avifRGBImage& a, const avifRGBImage& b,
                            int64_t* diffSum, int64_t* absDiffSum,
                            int64_t* sqDiffSum, int64_t* maxAbsDiff) {
  (a.depth <= 8) ? getDiffSumAndSqDiffSum<uint8_t>(a, b, diffSum, absDiffSum,
                                                   sqDiffSum, maxAbsDiff)
                 : getDiffSumAndSqDiffSum<uint16_t>(a, b, diffSum, absDiffSum,
                                                    sqDiffSum, maxAbsDiff);
}
...
class YUVToRGBTest
    : public testing::TestWithParam<
          std::tuple</*rgbDepth=*/int, /*yuvDepth=*/int, avifRGBFormat,
                     avifPixelFormat, avifRange, avifMatrixCoefficients,
                     /*addNoise=*/bool, /*rgbStep=*/uint32_t,
                     /*maxAbsAverageDiff=*/double, /*minPsnr=*/double>> {};

// Converts from RGB to YUV and back to RGB for all RGB combinations, separated
// by a color step for reasonable timing. If addNoise is true, also applies some
// noise to the input samples to exercise chroma subsampling.
TEST_P(YUVToRGBTest, Convert) {
  ...
  const bool isMonochrome =
      (yuvFormat ==
       AVIF_PIXEL_FORMAT_YUV400);  // If true, only test greyish input.
  ...
  std::unique_ptr<avifImage, decltype(&avifImageDestroy)> yuv(
      avifImageCreate(width, height, yuvDepth, yuvFormat), avifImageDestroy);
  ...
  int64_t diffSum = 0, absDiffSum = 0, sqDiffSum = 0, maxAbsDiff = 0,
          numDiffs = 0;
  const uint32_t maxValue = rgbMax - (addNoise ? 15 : 0);
  for (uint32_t r = 0; r < maxValue + rgbStep; r += rgbStep) {
    r = std::min(r, maxValue);  // Test the maximum sample value even if it is
                                // not a multiple of rgbStep.
    setImageChannel(&srcRgb, avifChannel::R, r,
                    addNoise ? redNoise : plainColor);

    if (isMonochrome) {
      // Test only greyish input when converting to a single channel.
      setImageChannel(&srcRgb, avifChannel::G, r,
                      addNoise ? greenNoise : plainColor);
      setImageChannel(&srcRgb, avifChannel::B, r,
                      addNoise ? blueNoise : plainColor);

      ASSERT_EQ(avifImageRGBToYUV(yuv.get(), &srcRgb), AVIF_RESULT_OK);
      ASSERT_EQ(avifImageYUVToRGB(yuv.get(), &dstRgb), AVIF_RESULT_OK);
      getDiffSumAndSqDiffSum(srcRgb, dstRgb, &diffSum, &absDiffSum, &sqDiffSum,
                             &maxAbsDiff);
      numDiffs += srcRgb.width * srcRgb.height * 3;  // Alpha is lossless.
    } else {
      for (uint32_t g = 0; g < maxValue + rgbStep; g += rgbStep) {
        g = std::min(g, maxValue);
        setImageChannel(&srcRgb, avifChannel::G, g,
                        addNoise ? greenNoise : plainColor);
        for (uint32_t b = 0; b < maxValue + rgbStep; b += rgbStep) {
          b = std::min(b, maxValue);
          setImageChannel(&srcRgb, avifChannel::B, b,
                          addNoise ? blueNoise : plainColor);

          ASSERT_EQ(avifImageRGBToYUV(yuv.get(), &srcRgb), AVIF_RESULT_OK);
          ASSERT_EQ(avifImageYUVToRGB(yuv.get(), &dstRgb), AVIF_RESULT_OK);
          getDiffSumAndSqDiffSum(srcRgb, dstRgb, &diffSum, &absDiffSum,
                                 &sqDiffSum, &maxAbsDiff);
          numDiffs += srcRgb.width * srcRgb.height * 3;  // Alpha is lossless.
        }
      }
  ...
  // paths. See reformat_libyuv.c.
  //       Slower non-libyuv conversions in libavif have a higher precision
  //       (using floating point operations for example).
  const double averageDiff =
      static_cast<double>(diffSum) / static_cast<double>(numDiffs);
  const double averageAbsDiff =
      static_cast<double>(absDiffSum) / static_cast<double>(numDiffs);
  ...
  static constexpr const char* avifRGBFormatToString[] = {
      "RGB", "RGBA", "ARGB", "BGR", "BGRA", "ABGR"};
  std::cout << " RGB " << rgbDepth << " bits, YUV " << yuvDepth << " bits, "
            << avifRGBFormatToString[rgbFormat] << ", "
            << avifPixelFormatToString(yuvFormat) << ", "
            << (yuvRange ? "full" : "lmtd") << ", MC " << matrixCoefficients
            << ", " << (addNoise ? "noisy" : "plain") << ", avg " << averageDiff
            << ", abs avg " << averageAbsDiff << ", max " << maxAbsDiff
            << ", PSNR " << psnr << "dB" << std::endl;
}

constexpr avifRGBFormat allRgbFormats[] = {
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Combine(
        /*rgbDepth=*/Values(8),
        /*yuvDepth=*/Values(8), ValuesIn(allRgbFormats),
...
INSTANTIATE_TEST_SUITE_P(
    All8b, YUVToRGBTest,
    Combine(/*rgbDepth=*/Values(8),
            /*yuvDepth=*/Values(8, 10, 12), ValuesIn(allRgbFormats),
            Values(AVIF_PIXEL_FORMAT_YUV444, AVIF_PIXEL_FORMAT_YUV422,
                   AVIF_PIXEL_FORMAT_YUV420),
            ...
            /*maxAbsAverageDiff=*/Values(1.),  // Not very accurate because high
                                               // rgbStep.
            /*minPsnr=*/Values(36.)));
INSTANTIATE_TEST_SUITE_P(
    All10b, YUVToRGBTest,
    Combine(/*rgbDepth=*/Values(10),
            ...
            /*maxAbsAverageDiff=*/Values(0.03),  // Not very accurate because
                                                 // high rgbStep.
            /*minPsnr=*/Values(47.)));
INSTANTIATE_TEST_SUITE_P(
    All12b, YUVToRGBTest,
