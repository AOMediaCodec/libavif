                                            ...
                                            avifBool expectIsoGainMap)
{
    ...
    for (jpeg_saved_marker_ptr marker = cinfo->marker_list; marker != NULL; marker = marker->next) {
        ...
        if ((marker->marker == (JPEG_APP0 + 2)) && (marker->data_length > tagMpf.size) &&
            ...
            if (!avifJPEGExtractGainMapImageFromMpf(f, sizeLimit, &mpfData, image, chromaDownsampling, requestedFormatGainMap)) {
                if (f == stdin) {
                                     ...
                                     uint32_t sizeLimit)
{
    ...
    if (!ignoreGainMap && hasMpfSegment(&cinfo)) {
        ...
        if (avifJPEGExtractGainMapImage(f, sizeLimit, &cinfo, avif, gainMap, chromaDownsampling, requestedFormatGainMap, expectIsoGainMap)) {
